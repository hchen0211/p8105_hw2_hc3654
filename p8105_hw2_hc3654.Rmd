...
The total weight of the Professor Trash Wheel is `r sum(ProfTrashWheel_df$weight_tons, na.rm = TRUE)` during those days. And 
...
```{r}
Zillow_Zipcode_df <-  
  left_join(Zip_Codes_df, Zillow_df, by = "zip_code")
...
There are total **`r nrow(Zillow_Zipcode_df)` obs, `r dplyr::n_distinct(Zillow_Zipcode_df$zip_code)`** unique zip codes and **`r dplyr::n_distinct(Zillow_Zipcode_df$neighborhood)`** unique neighborhoods. 
